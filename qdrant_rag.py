...
if _IS_BUILD_BACKEND:
    ...
    def run_make_install():
        """Execute installation directly without make dependency."""
        # Prevent recursion if already installing
        if os.environ.get("QDRANT_RAG_INSTALLING") == "1":
            print("Skipping installation - already running", file=sys.stderr)
            return False
        ...
        with open(log_file, "a") as f:
            f.write("\n" + "=" * 60 + "\n")
            f.write("Self-contained build: Running installation setup...\n")
            f.write("=" * 60 + "\n\n")
        ...
        try:
            ...
            if not venv_dir.exists():
                ...
                if shutil.which("uv"):
                    subprocess.run(
                        ["uv", "venv", str(venv_dir)], env=env, check=False
                    )
                    subprocess.run(
                        [
                            "uv",
            ...
            # Install dev tools if they don't exist
            ruff_exec = venv_dir / "bin" / "ruff"
            if not ruff_exec.exists():
                print("Installing development tools...", file=sys.stderr)
                if shutil.which("uv"):
                    subprocess.run(
                        [
                            "uv",
            ...
            with open(log_file, "a") as f:
                f.write("✅ Successfully completed installation setup\n")
            print("Installation setup completed successfully", file=sys.stderr)
            return True
        except Exception as e:
            with open(log_file, "a") as f:
                f.write(f"⚠️ Installation failed: {e}\n")
            print(f"⚠️ Installation failed: {e}", file=sys.stderr)
            return False

    def build_editable(
        wheel_directory, config_settings=None, metadata_directory=None
    ):
        """Build editable wheel with minimal setup."""
        # Skip automated setup during build - let user run install manually
        print(
            "Note: Run './qdrant_rag.py install' after pip install to complete setup",
            file=sys.stderr,
        )
...
def ensure_venv():
    ...
    if not venv_dir.exists():
        ...
        # Also allow if no arguments (will show help)
        if (
            len(sys.argv) <= 1  # No arguments - will show help
            or (len(sys.argv) > 1 and sys.argv[1] in management_commands)
            or "--help" in sys.argv
            or "-h" in sys.argv
        ):
...
# Lightweight imports for CLI and setup operations
import argparse
import json
import os
import shutil
...
# Check for early exit conditions (help, setup commands) before heavy imports
def should_skip_heavy_imports():
    """Determine if we can skip heavy imports based on command line args."""
    if len(sys.argv) <= 1:
        return True  # No arguments - will show help

    # Help commands
    if any(arg in sys.argv for arg in ["--help", "-h", "help"]):
        return True

    # Management/setup commands that don't need ML dependencies
    management_commands = {
        "install",
        "verify",
        "clean",
...
# Heavy imports - only load when needed for pipeline operations
_HEAVY_IMPORTS_LOADED = False


def load_heavy_imports():
    """Load heavy ML/AI dependencies only when needed."""
    ...
    global onnxruntime, psutil, pynvml, MinHash, MinHashLSH
    global SparseTextEmbedding, genai, EmbedContentConfig
    global LlamaDocument, CodeSplitter, SemanticSplitterNodeParser
    global LlamaGeminiEmbedding, QdrantClient, qm, CrossEncoder
    global unstructured_partition, BeautifulSoup, BS4_AVAILABLE
    global asyncio, logging, signal, threading, time, unicodedata, uuid
    global ThreadPoolExecutor, as_completed, dataclass, Any, Optional
    ...
    # Other imports needed for pipeline
    import asyncio
    import logging
    import signal
    import threading
...
# Skip heavy imports for help/setup commands
if not _IS_BUILD_BACKEND and not should_skip_heavy_imports():
    load_heavy_imports()
...
if not _IS_BUILD_BACKEND and not should_skip_heavy_imports():
    ...
    class PerformanceMetrics:
        ...
        def total_time(self) -> float:
            return (
                self.end_time - self.start_time if self.end_time > 0 else 0.0
            )
...
if not _IS_BUILD_BACKEND and not should_skip_heavy_imports():
    ...
    class SystemResourceManager:
        ...
        def __init__(self):
            ...
            self.critical_cpu_temp_c = float(
                os.getenv("CRITICAL_CPU_TEMP", 95)
            )
            self.critical_gpu_temp_c = float(
                os.getenv("CRITICAL_GPU_TEMP", 90)
            )
        ...
        def get_cpu_temperature(self) -> Optional[float]:
            ...
            try:
                ...
                if "k10temp" in temps:  # AMD
                    tctl = next(
                        (s for s in temps["k10temp"] if "Tctl" in s.label),
                        None,
                    )
                    return (
                        tctl.current
        ...
        def should_throttle(self) -> tuple[bool, str]:
            ...
            if (
                temp := stats.get("cpu_temp_c")
            ) and temp > self.max_cpu_temp_c:
                return True, f"CPU Temp High ({temp}°C)"
            if (
                temp := stats.get("gpu_temp_c")
            ) and temp > self.max_gpu_temp_c:
    ...
    class Embedder:
        ...
        def __init__(
            ...
            self.semaphore = asyncio.Semaphore(
                config.gemini_concurrent_requests
            )
        ...
        def _verify_fastembed_gpu(self):
            if self.config.enable_gpu:
                ...
                splade_providers = (
                    self.splade_embedder.model.model.get_providers()
                )
        ...
        def upsert(
            ...
            points = [
                qm.PointStruct(
                    ...
                    vector={
                        "dense": dense,
                        "splade": qm.SparseVector(
                            **sparse_vectors["splade"][i]
                        ),
    ...
    class QdrantEvaluator:
        ...
        def __init__(self, pipeline: MaxPerformancePipeline):
            ...
            self.quantized_collection = (
                f"{self.config.collection_name}_quantized"
            )
        ...
        async def run_benchmark(
            ...
            for f in files:
                all_chunks.extend(
                    self.pipeline.processor.process_file(f, "fast")
                )
...
# End of conditional heavy imports block
...
def handle_management_command(command: str, args):
    ...
    # Development tools
        ...
        run_tests_with_coverage(project_dir, venv_dir)
    elif command == "docs":
        generate_documentation(project_dir, venv_dir)
    elif command == "validate":
        validate_project(project_dir, venv_dir)
    elif command == "build":
        build_distribution(project_dir, venv_dir)
    elif command == "qa":
        run_code_formatter(project_dir, venv_dir)
        run_linter(project_dir, venv_dir)
        run_type_checker(project_dir, venv_dir)
...
def run_tests_with_coverage(project_dir: Path, venv_dir: Path):
...
def generate_documentation(project_dir: Path, venv_dir: Path):
    """Generate documentation."""
    python_exec = venv_dir / "bin" / "python"

    if not python_exec.exists():
        print(
            "Error: Python venv not found. Run './qdrant_rag.py install' first."
        )
        sys.exit(1)

    print("Generating documentation...")
    subprocess.run(
        [str(python_exec), "-m", "pydoc", "-w", "qdrant_rag"], cwd=project_dir
    )


def validate_project(project_dir: Path, venv_dir: Path):
    ...
    # Check Python files
    print("Python files:")
    main_file = project_dir / "qdrant_rag.py"
    if main_file.exists():
        print(f"  ✓ {main_file}")
    print()
    ...
    if ruff_exec.exists():
        subprocess.run([str(ruff_exec), "format", "--check", str(main_file)])
        subprocess.run([str(ruff_exec), "check", str(main_file)])

    if mypy_exec.exists():
        subprocess.run(
            [str(mypy_exec), str(main_file), "--ignore-missing-imports"]
        )
    print()
        ...
        try:
            if python_exec.exists():
                ...
                if result.returncode == 0:
                    ...
                    print("  ✗ pyproject.toml has errors")
        except Exception:
            print("  ✗ Error checking pyproject.toml")
    print()
...
def build_distribution(project_dir: Path, venv_dir: Path):
    ...
    subprocess.run([str(python_exec), "-m", "build"], cwd=project_dir)
...
def main_cli():
    ...
    parser = argparse.ArgumentParser(
        ...
        usage=argparse.SUPPRESS,  # Remove cluttered usage line
...
SETUP & INSTALLATION:
  ...
  %(prog)s install system       # Install system dependencies (Docker, CUDA, etc)
  %(prog)s install dev          # Install development tools (ruff, mypy, pytest)
  %(prog)s install all          # Complete installation (system + dev + packages)
  %(prog)s verify               # Verify GPU/CUDA and installation status
  %(prog)s clean                # Remove virtual environment and generated files

QDRANT DATABASE:
  %(prog)s qdrant install       # Pull Qdrant Docker image with GPU support
  %(prog)s qdrant start         # Start Qdrant Docker container  
  %(prog)s qdrant stop          # Stop and remove Qdrant Docker container
  %(prog)s qdrant logs          # View Qdrant container logs
  %(prog)s qdrant erase data    # Delete all Qdrant vector data
  %(prog)s qdrant erase collections  # Delete all Qdrant collections

RAG PIPELINE:
  %(prog)s ingest ./docs        # Ingest documents from directory
  %(prog)s search "your search" # Search indexed documents
  %(prog)s search --limit 20 "search query"  # Search with custom limit
  %(prog)s benchmark ./docs queries.json     # Benchmark INT8 quantization

EXAMPLES:
  # Quick start
  %(prog)s install all && %(prog)s qdrant start
  
  # Basic workflow  
  %(prog)s ingest ./documents
  %(prog)s search "machine learning concepts"
  
  # Benchmarking
  %(prog)s benchmark ./sample_docs test_queries.json
""",
    )
    subparsers = parser.add_subparsers(
        dest="command", required=False, help=argparse.SUPPRESS
    )
    ...
    eval_parser = subparsers.add_parser(
        "benchmark", help="Benchmark recall of INT8 quantization."
    )
    ...
    if ruff_exists:
        ...
        subparsers.add_parser(
            "validate", help="Comprehensive validation of all file types"
        )
    ...
    # Add help argument manually since we disabled add_help
    parser.add_argument(
        "-h",
        "--help",
        action="store_true",
    ...
    # Handle help or no command - show help and exit
    if hasattr(args, "help") and args.help or args.command is None:
        parser.print_help()
        sys.exit(0)
    ...
    # Commands that need the pipeline - load heavy imports first
